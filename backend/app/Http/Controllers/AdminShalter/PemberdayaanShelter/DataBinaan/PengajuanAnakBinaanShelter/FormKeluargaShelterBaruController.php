...
class FormKeluargaShelterBaruController extends Controller
{
    public function formKeluargaBaru() {
        // Data admin shelter yang login
        $user_id = auth()->user()->id_users;
        $adminShalter = AdminShelter::where('user_id', $user_id)->first();
    
        if (!$adminShalter) {
            abort(403, 'Admin Shalter tidak ditemukan.');
        }
    
        // Ambil data yang relevan dengan admin shelter yang login
        $provinsi = Provinsi::all();
        $kacab = Kacab::where('id_kacab', $adminShalter->id_kacab)->get();
        $wilbins = Wilbin::where('id_kacab', $adminShalter->id_kacab)->get(); // Ambil data wilayah binaan
        $shelters = Shelter::where('id_wilbin', $adminShalter->id_wilbin)->get(); // Ambil data shelter berdasarkan wilayah binaan
        $bank = Bank::all();
    
        // Pass data ke view
        return view('AdminShalter.Pemberdayaan.DataBinaan.PengajuanAnakBinaan.FormPengajuanAnaKeluargaBaru', compact('provinsi', 'kacab', 'wilbins', 'shelters', 'bank', 'adminShalter'));
    }
   ...
   public function store(Request $request)
        {
            ...
            // Pastikan admin cabang hanya menyimpan data di cabang yang sesuai
            $user_id = auth()->user()->id_users;
            $adminShelter = AdminShelter::where('user_id', $user_id)->first();

            if (!$adminShelter || $adminShelter->id_kacab != $request->id_kacab) {
                abort(403, 'Anda tidak diizinkan untuk menyimpan data di cabang ini.');
            }
            ...
            DB::commit();
            return redirect()->route('form_keluarga_baru.shelter')->with('success', 'Data Pengajuan Anak Binaan Berhasil Ditambahkan.');
        } catch (\Exception $e) {
            DB::rollback();
