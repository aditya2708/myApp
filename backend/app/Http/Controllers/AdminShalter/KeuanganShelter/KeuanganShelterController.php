...
namespace App\Http\Controllers\AdminShalter\KeuanganShelter;
...
class KeuanganShelterController extends Controller
{
    public function index() {
        ...
        // Mengambil data keuangan yang terkait dengan shelter yang dimiliki admin shelter yang login
        $data_keuangan = Keuangan::whereHas('anak', function($query) use ($id_shelter) {
            $query->where('id_shelter', $id_shelter); // Filter berdasarkan id_shelter
        })->with('anak')->get();
    
        // Ambil data anak yang belum memiliki data keuangan dan masih aktif untuk shelter yang sesuai
        $available_anak = Anak::whereNotIn('id_anak', $data_keuangan->pluck('id_anak'))
                              ->where('status_validasi', 'Aktif')
                              ->where('id_shelter', $id_shelter) // Filter berdasarkan id_shelter
                              ->get();
    
        return view('AdminShalter.Keuangan.DataKeuangan.index', compact('data_keuangan', 'available_anak'));
    }
    ...
    public function store()
    {
        ...
        // Ambil data anak yang sesuai dengan shelter milik admin yang login
        $available_anak = Anak::where('id_shelter', $id_shelter)->with('donatur')->get();
        ...
        return view('AdminShalter.Keuangan.DataKeuangan.create', compact('available_anak', 'tingkat_sekolah_options', 'kelas_semester_options'));
    }
    ...
    public function storeprosess(Request $request)
    {
        ...
        return redirect()->route('datakeuangan.shelter')->with('success', 'Data keuangan berhasil ditambahkan.');
    }
    ...
    public function destroy($id_keuangan)
    {
        ...
        return redirect()->route('datakeuangan.shelter')->with('success', 'Data keuangan berhasil dihapus.');
    }
}
