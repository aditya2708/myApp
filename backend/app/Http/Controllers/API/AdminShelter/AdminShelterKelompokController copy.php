...
{
    $kelompok = Kelompok::with(['shelter', 'anak'])
        ->withCount('anak')
        ->findOrFail($id);
   ...
   */
   ...
   public function addChildToGroup(Request $request, $id_kelompok)
   {
       ...
       if (!$this->validateAnakKelasCompatibility($anak, $kelompok->kelas_gabungan)) {
           return response()->json([
               'success' => false,
               'message' => 'Tingkat pendidikan anak tidak sesuai dengan kelas gabungan kelompok'
           ], 400);
       }

       $anak->id_kelompok = $id_kelompok;
       // Removed id_level_anak_binaan assignment - using kelas_gabungan approach
       $anak->save();
   ...
   */
   ...
   public function getAvailableAnak($kelompokId)
   {
       try {
           ...
           if (!empty($kelompok->kelas_gabungan)) {
               ...
               $compatibleAnak = $availableAnak; // All anak are compatible if no specific kelas gabungan
           }
