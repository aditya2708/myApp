...
class DataPengajuanPBController extends Controller
{
    public function pengajuanDonaturPBCabang(Request $request) {
     ...
     return view('AdminCabang.Settings.PengajuanDonatur.PB.index', compact('data_penerima_beasiswa', 'donaturs', 'kacabs', 'wilbins', 'shelters'));
    }
    ...
    Public function update(Request $request, $id_anak) {
        ...
        return redirect()->route('pengajuan_donatur.cabang', [
    ...
    public function destroy($id_anak) {
        ...
        if (!$anak) {
            return redirect()->route('pengajuan_donatur.cabang')->with('error', 'Data tidak ditemukan.');
        }
        $anak->id_donatur = null;
        $anak->status_cpb = 'CPB';
        $anak->save();
        return redirect()->route('pengajuan_donatur.cabang')->with('success', 'Donatur berhasil dihapus');
    }
