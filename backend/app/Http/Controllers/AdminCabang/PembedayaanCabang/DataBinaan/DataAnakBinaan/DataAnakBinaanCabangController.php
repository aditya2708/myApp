...
namespace App\Http\Controllers\AdminCabang\PembedayaanCabang\DataBinaan\DataAnakBinaan;
...
class DataAnakBinaanCabangController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Query dasar untuk anak yang sudah diaktivasi dan sesuai dengan admin cabang
        $query = Anak::where('status_validasi', Anak::STATUS_AKTIF)
            ->with('keluarga') // Eager loading relasi keluarga
            ->whereHas('keluarga', function ($q) use ($adminCabang) {
                $q->where('id_kacab', $adminCabang->id_kacab);
            });

        // Filter berdasarkan Wilayah Binaan (id_wilbin)
        ...
        // Filter berdasarkan Shelter (id_shelter)
        ...
        // Dapatkan data berdasarkan filter yang diterapkan
        $data_anak = $query->get();

        // Ambil data untuk dropdown filter: Kacab dan Wilbin yang relevan dengan admin cabang yang login
        $kacab = Kacab::where('id_kacab', $adminCabang->id_kacab)->get();
        $wilbins = Wilbin::where('id_kacab', $adminCabang->id_kacab)->get();

        // Kembalikan data ke view
        return view('AdminCabang.Pemberdayaan.DataBinaan.DataAnakBinaan.index', compact('data_anak', 'kacab', 'wilbins'));
    }
    ...
    public function getAnakBinaanAjax(Request $request)
    {
        ...
        $data = $query->get()->map(function ($anak, $index) use ($request) {
            ...
            $showUrl = route('AnakBinaan.show.cabang', $anak->id_anak);
            $deleteAction = "confirmDelete('{$anak->full_name}', {$anak->id_anak})";
            $nonaktifAction = route('anak.nonactivasi.cabang', $anak->id_anak);
    ...
    public function show($id_anak) {
        ...
        return view('AdminCabang.Pemberdayaan.DataBinaan.DataAnakBinaan.show', compact('anak', 'tab'));
    }
    ...
    public function nonactivasi($id)
    {
        ...
        return redirect()->route('NonAnakBinaan.cabang')->with('success', 'Status validasi anak berhasil dinonaktifkan.');
    }

    public function editanakbinaanCabang($id)
    {
        ...
        return view('AdminCabang.Pemberdayaan.DataBinaan.DataAnakBinaan.DataAnakEditBinaan.AnakBinaanEdit', compact('anak', 'daftarKeluarga'));
    }

    // Menyimpan proses update data anak
    public function editprosessanakbinaanCabang(Request $request, $id)
    {
        $request->validate([
        ...
        return redirect()->route('AnakBinaan.show.cabang', $anak->id_anak)
                         ->with('success', 'Data anak berhasil diperbarui.');
    }

     // Data Pendidikan Anak
     public function editPendidikanAnakCabang($id) {
        $anak = Anak::with('anakPendidikan')->findOrFail($id); // Mengambil data anak beserta pendidikan
        return view('AdminCabang.Pemberdayaan.DataBinaan.DataAnakBinaan.DataPendidikanAnakEdit.DataPendidikanBinaanEdit', compact('anak'));
    }

    public function editprosesspendidikananakCabang(Request $request, $id)
    {
        $request->validate([
        ...
        return redirect()->route('AnakBinaan.show.cabang', ['id' => $anak->id_anak, 'tab' => 'anak-pendidikan'])
