...
const normalizeFilters = (filters = {}, defaults = DEFAULT_FILTERS) => ({
  ...
  jenis_kegiatan: filters.jenis_kegiatan ?? defaults.jenis_kegiatan ?? DEFAULT_FILTERS.jenis_kegiatan
});
...
const formatDateLabel = (value) => {
  ...
  try {
    ...
    // ignore parsing issues and fallback to raw string
  }

  return value;
};

const AchievementReportFilters = ({
  visible,
  filters,
  defaultFilters,
  onClose,
  ...
  const jenisChoices = useMemo(() => {
    if (!Array.isArray(jenisOptions) || jenisOptions.length === 0) {
      return [{ value: null, label: 'Semua Jenis' }];
    }

    const hasAll = jenisOptions.some(option => option.value === null);
    if (hasAll) {
      return jenisOptions;
    }

    return [{ value: null, label: 'Semua Jenis' }, ...jenisOptions];
  }, [jenisOptions]);
        ...
        <View style={styles.content}>
          ...
          <View style={styles.section}>
            ...
            >
              ...
              <Ionicons name="calendar-outline" size={20} color="#7b8794" />
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.optionGrid}>
              {jenisChoices.map(option => {
                const isActive = localFilters.jenis_kegiatan === option.value;
...
export default AchievementReportFilters;
